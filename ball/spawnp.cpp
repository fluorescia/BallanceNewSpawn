...
void spawnp::OnModifyConfig(CKSTRING category, CKSTRING key, IProperty* prop) {
	enabled = prop_enabled->GetBoolean();
	reset_enabled = reset_prop_enabled->GetBoolean();
	spirit_enabled = reset_spirit_enabled->GetBoolean();
	if (prop == prop_key) {
...
void spawnp::OnLoadScript(CKSTRING filename, CKBehavior* script) {
...
void spawnp::OnLoadObject(CKSTRING filename, BOOL isMap, CKSTRING masterName, CK_CLASSID filterClass, BOOL addtoscene, BOOL reuseMeshes, BOOL reuseMaterials, BOOL dynamic, XObjectArray* objArray, CKObject* masterObj)
{
	...
	if (input_manager->IsKeyPressed(key_prop) && Ball_Active) {
		...
		if (m_curLevel) {
			if (set_sector != cur_sector) {
				...
				m_bml->AddTimerLoop(1u, [this, set_sector, sectorMgr, ctx]() {
					if (sectorMgr->IsActive())
						return true;

					m_bml->AddTimer(2u, [this, set_sector, ctx]() {
						CKBOOL active = false;
						...
						if (set_sector > m_checkpoints->GetRowCount()) {
							...
							m_bml->AddTimer(2u, [this, set_sector, ctx, flame]() {
								VxMatrix matrix;
		...
		m_bml->AddTimer(2u, [this, current_ball, mm, ballDeact]() {
